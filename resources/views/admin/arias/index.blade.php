    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        <div id="kt_content_container" class="container">
            <div class="card mb-5 mb-xl-8">
                ...
                <span
                    class="card-label fw-bold fs-3 mb-1">{{ trans('global.view') }} {{ trans('global.arias') }}</span>
                    </h3>
                    <div class="card-toolbar">
                        <a href=" " class="btn btn-sm btn-light-primary" data-bs-toggle="modal"
                           data-bs-target="#kt_modal_new_target">
                            <span class="svg-icon svg-icon-2 bi-bag-plus"></span>
                            {{ trans('global.add') }} {{ trans('global.aria_name') }}
                        </a>
                    </div>
                        ...
                        <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                            ...
                            @foreach ($arias  as $key => $aria)
                                <tr>
                                    ...
                                    <td>
                                        <a href="#"
                                           class="text-dark fw-bold text-hover-primary d-block mb-1 fs-6">{{ $aria->title ?? '' }}</a>
                                    </td>


                                    <td class="text-end">
                                        <a href="{{ route('admin.cities.index', $aria->id) }}">
                                        ...
                                        <a href="{{ route('admin.arias.edit', $aria->id) }}"
                                           class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                            @include('partials.icons.edit')
                                        </a>

                                        <button category_id_attr="{{$aria->id}}"
                                                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm"
                                                onclick="Confirm_Delete(this)"
                                                data-url="{{ route('admin.arias.destroy', $aria->id) }}">
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '#btn_save_user', function (e) {
            ...
            e.preventDefault();
            $('.errors').text('');
            var formData = new FormData($('#formSaveAria')[0]); //get all data in form
            $.ajax({
                type: 'post',
                enctype: 'multipart/form-data',
                url: "{{ route("admin.arias.store",$countryId) }}",
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        ...
                        document.getElementById("formSaveAria").reset();
                        setTimeout(function () {
                            var url = "{{ route('admin.arias.index',$countryId) }}"; //the url I want to redirect to
