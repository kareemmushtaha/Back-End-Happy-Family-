...
@section('title',trans('global.edit') .' '. $story->name)
@section('content')
        ...
        <div class="container">
            ...
            <div class="card card-custom example example-compact">
                <div class="card-header">
                    <h3 class="card-title">{{ trans('global.edit') }} {{ $story->name }}</h3>
                ...
                <form class="form fv-plugins-bootstrap fv-plugins-framework" id="formSuccessStory"
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-10">
                            <div>
                                <label class=" mb-2">{{__('lang.image')}}</label>
                            </div>
                            <div class="image-input image-input-outline user_edit_image" data-kt-image-input="true"
                                ...
                                @if($story->photo)
                                    <div class="image-input-wrapper user_edit_image w-125px h-125px"
                                         style="background-image: url({{$story->photo}});"></div>
                                @else
                                    <div class="image-input-wrapper user_edit_image w-125px h-125px"
                        ...
                        <div class="mb-10">
                            ...
                            <input type="text" class="form-control"
                                   ...
                                   name="name" id="name" value="{{$story->name ?? old('name')}}" />
                        ...
                        <div class="mb-10">
                            ...
                            <input type="text" class="form-control"
                                   ...
                                   name="title" id="title" value="{{$story->title ?? old('title')}}" />
                            <span class="text-danger errors"
                                  id="title_error"> </span>
                        </div>
                        ...
                        <div class="mb-10">
                            ...
                            <textarea name="description" class="form-control" rows="6" id="description">{{$story->description ?? old('description')}}</textarea>
                            <span class="text-danger errors"
                        ...
                        <div>
                            ...
                            <select class="form-control" name="status" id="status" required>
                                @foreach($success_stories_status as $key => $status)
                                    <option value="{{ $key }}" @if($key == $story->status) selected @endif>{{ $status }}</option>
                                @endforeach
                            </select>
                            <span class="text-danger errors"
    ...
    <script>
        ...
        $(document).on('click', '#btn_save_success_story', function (e) {
            ...
            $.ajax({
                type: 'post',
                enctype: 'multipart/form-data',
                url: "{{ route("admin.success-stories.update", $story->id) }}",
