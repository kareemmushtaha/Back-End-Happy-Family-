    ...
    <div class="d-flex flex-column-fluid">
        ...
        <div class="container">
            ...
            <div class="card card-custom example example-compact">
                ...
                <!--begin::Form-->
                <form class="form fv-plugins-bootstrap fv-plugins-framework" id="formUpdateQuestionChat"
                    ...
                    <div class="card-body">
                        ...
                        <div>
                            ...
                            <select class="form-control" name="status" id="status" multiple required>
                                ...
                                <option value="1"
                                        @if($question->status ==1) selected @endif>{{trans('global.active')}}</option>
                                <option value="0"
                                        @if($question->status ==0) selected @endif>{{trans('global.un_active')}}</option>
                            ...
                            <div class="row">
                                <div class="col-lg-9 ml-lg-auto">
                                    ...
                                    <button class="btn btn-primary mr-2" type="button"
                                            id="btn_update_question"> {{ trans('global.edit') }}</button>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#btn_update_question', function (e) {
            e.preventDefault();
            $('.errors').text('');
            ...
            $.ajax({
                type: 'post',
                enctype: 'multipart/form-data',
                url: "{{ route("admin.personal-questions.update", $question->id) }}",
                data: formData, // send this data to controller
                processData: false,
                contentType: false,
                cache: false,
                success: function (data) {
                    $('#btn_update_question').html();
                    if (data.status == true) {
                        Swal.fire({
                            title: data.msg,
                            text: data.msg,
