...
@section('script')
    ...
    <script>
        ...
        $(document).on('keyup', '#search', function () {
            ...
            $.ajax({
                type: "post",
                url: "{{route('mediator.chat.searchMessages')}}",
                data: {'search': search},
                dataType: "json",
                success: function (response) {
                    $('#users-links-chat').html('')
                    var user_id = {{$user_follow_mediator}};
        ...
        function setUsersLink() {
            $.ajax({
                type: "GET",
                url: "{{route('mediator.chat.getMessages',$user_follow_mediator)}}",
                dataType: "json",
                success: function (response) {
                    $('#users-links-chat').html('')
                    var user_id = {{$user_follow_mediator}};
        ...
        function getMessages() {
            if (chat_id != 0)
                $.get("{{url('mediator/follow-mediator-chat/chat')}}/" + user_id + '/' + chat_id, function (response) {
                    ...
                    appendOtherUserCard(response.other_user),
                        $.each(response.messages, function (key, value) {
                            ...
                            if (user_id !== value.received_id) {
                                ...
                                var answer_id = `answer_conversation${value.id}{{$user_follow_mediator}}`;
                            }
        ...
        function SendAnswer() {
            ...
            $.ajax({
                url: '{{ route('mediator.chat.send_answer_chat',$user_follow_mediator) }}',
                type: 'get',
                data: {
                    '_token': token,
                ...
                success: function (response) {
                    if (response.status == true) {
                        ...
                        document.getElementById(`answer_conversation${response.conversation_id}{{$user_follow_mediator}}`).innerHTML = response.answer_title;
        ...
        function SendCustomAnswer() {
            ...
            var token = '{{csrf_token()}}';
            $.ajax({
                url: '{{ route('mediator.chat.send_custom_answer_chat',$user_follow_mediator) }}',
                type: 'get',
                data: {
                    '_token': token,
                    'question_id': question_id,
        ...
        function SendCustomQuestion() {
            ...
            var chat_id = $('#chat_id').val();
            $.ajax({
                url: '{{ route('mediator.chat.send_custom_question_chat',$user_follow_mediator) }}',
                type: 'get',
                data: {
                    '_token': token,
        ...
        function appendOtherUserCard(response) {
            ...
            card = ` <div class="user-profile d-flex flex-column align-items-center">` +
                ...
                `<a href=""><h1 style="color: #FF7171;">انت الأن تتحدث بنيابة عن {{\App\Models\User::query()->find($user_follow_mediator)->first_name}} </h1></a>`;
